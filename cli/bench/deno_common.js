...
Deno.bench(
  "op_void_async",
  async () => Deno.core.opAsync("op_void_async"),
);
...
{
  let i = 0;
  Deno.bench("url_parse", () => {
    new URL(`http://www.google.com/${i}`);
    i++;
  });
}

Deno.bench("blob_text_large", () => {
  new Blob([input]).text();
});

const input = "long-string".repeat(99999);
Deno.bench("b64_rt_long", () => {
  atob(btoa(input));
});

Deno.bench("b64_rt_short", () => {
  atob(btoa("123"));
});
...
Deno.bench(
  "read_128k",
  async () => Deno.readFile("./cli/bench/testdata/128k.bin"),
);

Deno.bench("request_new", () => new Request("https://deno.land"));
